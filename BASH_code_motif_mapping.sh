...
#6A: The name of the TF in DAP-SEQ paper is buried in a file name annotation, we need to extract this. 
echo 'Annotating Hits.'
echo `awk '{OFS="\t"}{split($1,a,"."); split(a[2],b,"_");print b[1],$2,$3,$4,$5,$6,$7,$8,$9}' ${OUTFILE_ALL} > ${OUTFILE_NAMES}`

#Step 6: Convert Output .txt File to .bed File
echo 'Converting Mapping Files to BED Format.'
echo `awk '{OFS="\t"}{split($2,a,":");split(a[2],b,"-");print a[1],b[1],b[2],$1}' ${OUTFILE_NAMES} > ${OUTFILE_ALL_BED}`
