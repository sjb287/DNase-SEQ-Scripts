...
# gff3File = args[2]
# Species  = args[3]
# MACSFILE = args[4] # MACS peaks file (xls file converted to csv)
...
col_names<-c("chr","start","end","ChrSizesNamed","p.value","strand")
names(bed)<-col_names
...
o <-data.frame(Feature=rownames(c$percentage),Percentage=c$percentage,row.names=NULL)
